...
class alm_filter_nlev_wl(opfilt_base.alm_filter_wl):
    def __init__(self, ninv_geom:utils_geom.Geom, nlev_t:float or np.ndarray, transf:np.ndarray, unlalm_info:tuple, lenalm_info:tuple, verbose=False, rescal=None,
        ...
        self.nlev_t_extended = _extend_cl(nlev_t_extended, lmax_sol)
        self.inoise_2_extended = _extend_cl(transf_extended ** 2, lmax_sol) * cli(self.nlev_t_extended ** 2) * (180 * 60 / np.pi) ** 2
        self.inoise_1_extended = _extend_cl(transf_extended ** 1, lmax_sol) * cli(self.nlev_t_extended ** 2) * (180 * 60 / np.pi) ** 2
        self.transf_extended = _extend_cl(transf_extended, lmax_sol)
    ...
    def get_ftl(self, len=False, extended=False):
        if len:#FIXME
            ...
            return ret
        return np.copy(self.inoise_2) if not extended else np.copy(self.inoise_2_extended)
