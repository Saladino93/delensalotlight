...
from lenspyx.utils_hp import almxfl, Alm, synalm, alm2cl, alm_copy
...
class alm_filter_nlev_wl(opfilt_base.alm_filter_wl):
    def __init__(self, nlev_t:float or np.ndarray, nlev_p:float or np.ndarray, ffi:remapping.deflection, transf:np.ndarray,
                 ...
                 operators: secondaries.Operators = None, operators_temperature: secondaries.Operators = None, ninv_geom = None):
        ...
        #self.operators_temperature = operators_temperature
    ...
    def apply_alm(self, telm:np.ndarray):
        ...
        eblm = self.operators(telm[1:], lmax_in = self.mmax_sol, spin = 2, lmax_out = self.lmax_len, mmax_out = self.lmax_len,
                              backwards=False, q_pbgeom = self.ninv_geom)
        
        import healpy as hp

        print("LMAAAX", hp.Alm.getlmax(eblm[0].size))

        """tlm  = self.operators_temperature(telm[1:], lmax_in = self.mmax_sol, spin = 0, lmax_out = self.lmax_len, mmax_out = self.lmax_len,
                              backwards=False, q_pbgeom = self.ninv_geom)
        ...
        import healpy as hp
        print("LMAXES", hp.Alm.getlmax(eblm[0].size), self.mmax_len, self.mmax_sol)
        ...
        self.operators(eblm, lmax_in = self.mmax_len, spin = 2, lmax_out = self.lmax_sol, mmax_out = self.mmax_sol,
                                 backwards=True, gclm_out=telm[1:], out_sht_mode='GRAD_ONLY', q_pbgeom = self.ninv_geom, apply_weights = True)
        ...
        import healpy as hp
        print("LMAXXX", hp.Alm.getlmax(telm[0].size), self.mmax_sol, self.lmax_sol)
    ...
    def get_qlms_birefringence(self, eblm_dat: np.ndarray or list, elm_wf: np.ndarray, q_pbgeom: pbdGeometry, alm_wf_leg2:None or np.ndarray =None, which = "a", shift_1: int = 0, shift_2: int = 0):
        ...
        eblm_dat_map = self._get_irespmap(eblm_dat, elm_wf, q_pbgeom, which = which)#, map_out=resmap_r) # inplace onto resmap_c and resmap_r

        eblm_wf_map = self._get_gpmap(elm_wf, 2, q_pbgeom, which = which)  # 2 pos.space maps, uses then complex view onto real array
        gc = eblm_dat_map[0] * eblm_wf_map[0] + eblm_dat_map[1] * eblm_wf_map[1] #is this correct?
        lmax_qlm, mmax_qlm = self.operators.lmax(which = which), self.operators.mmax(which = which)
        gc *= (-1)**(which == "a")
        return q_pbgeom.geom.adjoint_synthesis(gc, 0, lmax_qlm, mmax_qlm, self.operators.sht_tr).squeeze()
    



    def get_qlms(self, teblm_dat: np.ndarray or list, telm_wf: np.ndarray, q_pbgeom: pbdGeometry, alm_wf_leg2:None or np.ndarray =None, which = "p", shift_1: int = 0, shift_2: int = 0):
        """
        If you want to get a disconnected part, you do this: 
        0.5*[qlms(shift_A, shift_B)+qlms(shift_B, shift_A)]
        """
        if which == "p":
            return self.get_qlms_lensing(teblm_dat, telm_wf, q_pbgeom, alm_wf_leg2, which = which, shift_1 = shift_1, shift_2 = shift_2)
        elif which in ["a", "f"]:
            return self.get_qlms_birefringence(teblm_dat[1:3], telm_wf[0], q_pbgeom, alm_wf_leg2, which = which, shift_1 = shift_1, shift_2 = shift_2)
        else:
            raise NotImplementedError


    def get_qlms_lensing(self, teblm_dat: np.ndarray or list, telm_wf: np.ndarray, q_pbgeom: pbdGeometry, alm_wf_leg2:None or np.ndarray =None, which = "p", shift_1: int = 0, shift_2: int = 0):
        """Get lensing generaliazed QE consistent with filter assumptions
        ...
        resmap_c = np.empty((q_pbgeom.geom.npix(),), dtype=elm_wf.dtype)
        resmap_r = resmap_c.view(rtype[resmap_c.dtype]).reshape((resmap_c.size, 2)).T  # real view onto complex array
        self._get_irespmap(eblm_dat, elm_wf, q_pbgeom, map_out=resmap_r, which = which, shift = shift_1) # inplace onto resmap_c and resmap_r
        gcs_r = self._get_gpmap(elm_wf, 3, q_pbgeom.geom, which = which, shift = shift_2)  # 2 pos.space maps, uses then complex view onto real array
        gc_c = resmap_c.conj() * gcs_r.T.view(ctype[gcs_r.dtype]).squeeze()  # (-2 , +3)
        gcs_r = self._get_gpmap(elm_wf, 1, q_pbgeom.geom, which = which, shift = shift_2)
        ...
        #gc = q_pbgeom.geom.adjoint_synthesis(gc_r, 1, lmax_qlm, mmax_qlm, self.operators.sht_tr)
    ...
    def get_qlms_old(self, teblm_dat: np.ndarray or list, telm_wf: np.ndarray, q_pbgeom: pbdGeometry, alm_wf_leg2:None or np.ndarray =None):
        """Get lensing generaliazed QE consistent with filter assumptions

            Args:
                eblm_dat: input temperature-polarization maps (geom must match that of the filter)
                elm_wf: Wiener-filtered CMB maps (alm arrays)
                alm_wf_leg2: Wiener-filtered CMB maps of gradient leg, if different from ivf leg (alm arrays)
                q_pbgeom: scarf pbounded-geometry of for the position-space mutliplication of the legs
    ...
    def _get_irespmap(self, eblm_dat:np.ndarray, eblm_wf:np.ndarray, q_pbgeom:pbdGeometry, map_out=None, which = "p", shift: int = 0):
...
def calc_prep(teblm:np.ndarray, s_cls: dict, ninv_filt: alm_filter_nlev_wl):
    ...
    telm[1:] = ninv_filt.operators(eblmc[1:], lmax_in = ninv_filt.mmax_len, spin = 2, lmax_out = ninv_filt.lmax_sol, mmax_out = ninv_filt.mmax_sol,
